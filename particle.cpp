...
#define	MAX_PARTICLE		(100000)		// パーティクル最大数
...

	{
		for (int nCntParticle = 0; nCntParticle < MAX_PARTICLE; nCntParticle++)
		{
			...
			{// 使用中
				...
				g_aParticle[nCntParticle].rot.z += 0.1f;
				...
				g_aParticle[nCntParticle].move.y += 0.15f;
				...
				else
				{
					if (g_aParticle[nCntParticle].nLife <= 8)
					{
						...
						g_aParticle[nCntParticle].material.Diffuse.y = 0.6f - (float)(80 - g_aParticle[nCntParticle].nLife) / 80 * 0.6f;
						g_aParticle[nCntParticle].material.Diffuse.z = 0.1f - (float)(80 - g_aParticle[nCntParticle].nLife) / 80 * 0.1f;
					}

					if (g_aParticle[nCntParticle].nLife <= 2)
					{
						// α値設定
		...
		if (enemy[0].hp > 0)
		{
			...
			pos.x -= sinf(enemy[0].rot.y + XM_PI) * 5.0f;
			pos.z -= cosf(enemy[0].rot.y + XM_PI) * 5.0f;

			float ang = (float)(rand() % 314 - 157) / 100.0f;

			fAngle = (enemy[0].rot.y + XM_PI) + ang;

			fLength = rand() % (int)(g_fWidthBase * 20) / 100.0f - g_fWidthBase;
			move.x = sinf(fAngle) * fLength;
			move.y = rand() % 30 / 100.0f + g_fHeightBase;
			...
			// ビルボードの設定
			SetParticle(pos, move, XMFLOAT4(0.8f, 0.3f, 0.1f, 1.0f), fSize, fSize, nLife);
		}
	}
}
...
void DrawParticle(void)
{
	...
	for (int nCntParticle = 0; nCntParticle < MAX_PARTICLE; nCntParticle++)
	{
		if (g_aParticle[nCntParticle].use)
		{
			...
			// 回転を反映
			mtxViewRot = XMMatrixRotationRollPitchYaw(g_aParticle[nCntParticle].rot.x, g_aParticle[nCntParticle].rot.y, g_aParticle[nCntParticle].rot.z);
...
int SetParticle(XMFLOAT3 pos, XMFLOAT3 move, XMFLOAT4 col, float fSizeX, float fSizeY, int nLife)
{
	...
	for (int nCntParticle = 0; nCntParticle < MAX_PARTICLE; nCntParticle++)
	{
		if (!g_aParticle[nCntParticle].use)
		{
			...
			g_aParticle[nCntParticle].scale = { fSizeX, fSizeY, fSizeY };
