...
struct DISSOLVE
{
	XMFLOAT4 dissolveColor; // ディゾルブ中のエフェクト色
	float DissolveTime;
	int DissolveFlug;
...
static ID3D11ShaderResourceView* noiseTextureSRV;
static ID3D11Buffer*			g_TexBlendBuffer = NULL;
static ID3D11ShaderResourceView* blendTexture;
...
//ディゾルブ
void SetDissolve(int flug, float time)
{
	g_Dissolve.dissolveColor = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	g_Dissolve.DissolveFlug = flug;
	g_Dissolve.dissolveThreshold = time;

	GetDeviceContext()->UpdateSubresource(g_DissolveBuffer, 0, NULL, &g_Dissolve, 0, 0);
}

////マルチテクスチャ~1
//void SetTexBrend(int flug, float rate, int texNo)
//{
//	g_Texblend.TexBlendFlug = flug;
//	g_Texblend.TexBlendRate = rate;
//
//	// ブレンド用テクスチャ読み込み
//	D3DX11CreateShaderResourceViewFromFile(GetDevice(),
//		"data/TEXTURE/jwel.jpg",
//		NULL,
//		NULL,
//		&blendTexture,
//		NULL);
//	g_ImmediateContext->PSSetShaderResources(3, 1, &blendTexture);
//
//
//	GetDeviceContext()->UpdateSubresource(g_TexBlendBuffer, 0, NULL, &g_TexBlendBuffer, 0, 0);
//}
...
{
	g_Texblend.TexBlendFlug = flug;
	g_Texblend.TexBlendTime = time;
	g_Texblend.TexBlendRate = rate;

	GetDeviceContext()->UpdateSubresource(g_TexBlendBuffer, 0, NULL, &g_Texblend, 0, 0);
}


void SetFuchi(int flag, XMFLOAT2 time, XMFLOAT4 color)
{
	g_Fuchi.fuchiColor = color;
	g_Fuchi.fuchi = flag;
	g_Fuchi.fuchiTime = time;
...
HRESULT InitRenderer(HINSTANCE hInstance, HWND hWnd, BOOL bWindow)
{
	...
	// ノイズテクスチャ
	D3DX11CreateShaderResourceViewFromFile(GetDevice(),
	"data/TEXTURE/noise.jpg",
	NULL,
	NULL,
	&noiseTextureSRV,
	NULL);
	g_ImmediateContext->PSSetShaderResources(1, 1, &noiseTextureSRV);

	// ブレンド用テクスチャ読み込み
	D3DX11CreateShaderResourceViewFromFile(GetDevice(),
		//"data/TEXTURE/jwel.jpg",
		"data/TEXTURE/rainbow_jwel.jpg",
		NULL,
		NULL,
		&blendTexture,
		NULL);
	...
	//テクスチャーブレンド
	hBufferDesc.ByteWidth = sizeof(TEXBLEND);
	g_D3DDevice->CreateBuffer(&hBufferDesc, NULL, &g_TexBlendBuffer);
	g_ImmediateContext->VSSetConstantBuffers(9, 1, &g_TexBlendBuffer);
	g_ImmediateContext->PSSetConstantBuffers(9, 1, &g_TexBlendBuffer);
	...
	//g_ImmediateContext->PSSetShaderResources(0, 1, &diffuseTextureSRV);
	//g_ImmediateContext->PSSetShaderResources(1, 1, &noiseTextureSRV);
...
void UninitRenderer(void)
{
	...
	if (g_TexBlendBuffer)		g_TexBlendBuffer->Release();
