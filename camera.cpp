...
void SetCameraAT(XMFLOAT3 pos)
{
	...
	float pulsrot;
	if (player->moveRot == PLAYER_RIGHT)pulsrot = XM_PI / 6;
	else if (player->moveRot == PLAYER_LEFT)pulsrot = -(XM_PI / 6);

	g_Camera.pos.x = player->pos.x - sinf(player->rot.y + XM_PI + pulsrot) * g_Camera.len;
	g_Camera.pos.y = player->pos.y + 10.0f;
	g_Camera.pos.z = player->pos.z - cosf(player->rot.y + XM_PI + pulsrot) * g_Camera.len;
